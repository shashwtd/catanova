...
export function historyTokens(line: string, names: readonly string[]): ReactNode[] {
  ...
  const actor = people.find(
    (name) =>
      ...
      /^(?: (?:is willing|declined|placed|built|bought|rolled|offered|traded|proposed|withdrew|played|collected|received|moved|discarded|took|claimed|settled|wins)\b|'s (?:turn|timer)\b)/.test(
