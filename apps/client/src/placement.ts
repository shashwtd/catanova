...
export function placementValid(
  ...
  if (
    (a.kind === 'road' || a.kind === 'ship') &&
    !building &&
    !['setupRoad', 'freeRoads'].includes(game.phase)
  )
    return false;
  if (a.kind === 'settlement' && !building && game.phase !== 'setupSettlement') return false;
  if (a.kind === 'city' && !building) return false;
  // Open Sea's ship move is made in the action phase only (docs/RULEBOOK-OPEN-SEA.md, section 8).
  if (a.kind === 'moveShip' && game.phase !== 'actions') return false;
  return a.kind === 'road'
