...
The inspected subscription reports a **sponsored offer** and `spendingLimit: Off`. Its remaining credits, expiry date, and any post-credit billing terms were **not established** by the APIs inspected. Confirm those in the sponsorship/billing portal. Other Azure workloads, domain renewal, wiki hosting, optional paid monitoring, and Supabase charges are outside this Catanova estimate. Azure cost alerts are not configured yet. Service alerts use a free external dead-man's-switch service at no Azure cost; they are ready to switch on as described in [Alerts](OPERATIONS.md#alerts).
...
- **Still pending:** real Google/guest sessions and account/invite flows, credit confirmation and measured player capacity. No wiki has been created. The reboot test used isolated data and does not establish real-user OAuth or a backup-based match recovery.
- **Built but not yet switched on (23 September 2026):**
  - **Monitoring alerts.** A five-minute watchdog (public HTTPS, container health, backup age, disks, TLS expiry) and dead-man's-switch pings from the backup worker and the restore drill. Each needs its free healthchecks.io check created and its URL pasted on the VM; see [Alerts](OPERATIONS.md#alerts).
  - **Recovery of a played match from a downloaded backup.** The weekly [restore drill](backup/README.md#restore-drill) does this: it verifies the newest backup with the deployed release, then checks every game's journal, rules invariants and next move. Locally it passed against really played test games, and a player resumed their seat on the real server from the verified copy. Its first run against a real production backup is still pending.
  - **The merge gate.** GitHub Actions has not run since 20 September 2026; `npm run ci` is now the local gate before merging and deploying.
