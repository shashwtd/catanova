...
- **Overview** is the page to glance at: people online now and playing now, live and paused games, open lobbies, games started and finished today and this week, and the accounts that played today and this week, with how many of them played their first game (the day and week start at the browser's own midnight and Monday). Below that, who is online and where (at a table with its room and turn, away from a seat they still hold, in a lobby, or elsewhere in Catanova), the live games with their players, points, turn (and this turn's roll, as two small dice) and running time, performance over the last hour, six hours or day, a line for each host report and the newest errors.
- **Growth** shows how Catanova is growing over the last 30 days, 90 days, year or all time (by day up to 90 days, by week up to two years, then by month; the range is in the link, such as `#/growth?range=1y`). Eight headline numbers each have a sparkline and, where the period before lies wholly after the first game, their change against it: accounts that have played, new and active players, games started and won, the median game, the share of seats bots filled, and new players back the next week. Below them: accounts over time, new players, active players and games with a moving average, game length (median and middle half), bot seats, and weekly retention as a heatmap of each week's new players and the share who played in each week after. The day, week or month still under way is drawn faint and left out of averages and sparklines, so it never reads as a fall. A player is an account that started a game; days, weeks and months are UTC. It is aggregates only, like Stats, so viewing it is not audited; the private retention report stays on Stats.
...
- **How the game went** is worked out from the game's journal and reports only what the table could see: the result and why the game ended, how long it took, turns and moves, standings with points, pieces, awards and each player's turn times, points per player turn by turn, the dice against what this game's dice mode expects (with each total's pair drawn under it and a grid of which pairs came up, read from the saved game after each roll), resources gained and spent by kind, the robber, trades, development cards, Longest Road and Largest Army changing hands, and bots, stand-ins and turn-timer moves.
...
- **Stats** has all-time and 30-day numbers; games started and ended per day side by side on one scale, with players per day and per week beside each other below (stacked on a phone); and the dice across every game, judged separately for each dice mode. Dice are drawn as the table draws them, the first ivory and the second teal: each total's most even pair under its column, and a six-by-six grid of which ordered pairs came up. Pairs are read from each roll's public line ("Ann rolled 3 + 4 = 7.", matched at the end of the line so a name cannot fake one) and kept only when they add up to the recorded total; any that do not are counted and said. Pairs are shown, never tested: a pair's expected count is given only where the dice mode fixes it exactly (one in 36 for Natural dice), not for the Balanced deck, which only comes close. **Feedback** is the players' inbox, and **Audit** the log of every change and private view.
- **Modes** says who may pick each game mode, and changes it while the server runs. Classic is always open; every other mode is **Off**, **Testers** (only rooms whose host is a tester) or **Everyone**, and opening one to everyone asks once more. Below that are the testers, each linked to their page, with who added them and when; a tester is added by account id here or with **Make tester** on a player's page, where a badge marks them. A change applies at once: the game reads the switches each time it needs them, and every lobby with someone connected is sent its room again, so a host's Game mode section appears or goes while they watch. Games already started keep their mode. `CATANOVA_MODES` and `CATANOVA_MODE_TESTERS` in `production.env` are only the starting point for what the console has not set, shown as such; a tester listed there cannot be removed here ([Game modes](GAME-MODES.md), "How the modes ship").
- **System** has the detail Overview leaves out: the process (and how much memory it may use: its container's limit where one is set, otherwise the machine's memory), event-loop delay, connections, rooms, the database and its journal, disk space, the host reports in full, recent server errors and refused admin requests.

Overview's performance charts and System's cards each carry a drawing of the thing they measure, drawn from its live reading: a processor whose die glows brighter and warmer with load, a memory stick whose chips fill with the share of memory in use, a stopwatch whose hand and arc show the event loop's delay, a plug pushed home (its light on) while sockets are open, a tank filled to the disk's use, the database, a settlement and a city whose windows are lit while games are played, and meeples for the people online (those playing in the players' colours, on a hex). The same drawings, dimmed and at rest, mark what is missing when nobody is online, no game is being played or there are no games yet. A reading that needs a look turns its drawing amber, then red, at the same thresholds as the rest of the console. They share one palette from the stylesheet's own colours and one light from the top left; they are decoration beside numbers that are always written out, and their small movements stop when the browser asks for reduced motion.

Every chart says exactly what each bar or point is: its whole date ("Tue 16 Sep", "Week of 15 Sep") or name ("Total 7") and each series' value there, on hover, on a tap (until the next tap elsewhere), and from the keyboard (Tab to the chart, then the arrow keys, Home, End and Escape, which a screen reader announces too). Axes label only the dates that fit, such as Mondays or the first of each month, and on a phone a long range narrows its columns to fine strokes so all of it, the latest last, fits the card. The numbers behind each chart are in its "Show the numbers" table or a table beside it.
...
5. **Audit.** Ending a game, resolving or reopening feedback, generating the retention report, changing who may pick a game mode (`mode.set`), adding or removing a tester (`mode.tester_add`, `mode.tester_remove`) and viewing a game's private state (hands and decks) each write a row to the append-only `admin_audit` table (triggers refuse updates and deletes) with the time, email, action, target, details, Cloudflare's client address, the request id and the Cloudflare ray id. A change and its audit row commit in the same transaction.
...
- **Opening or closing a game mode** is a click on the Modes tab, with no restart. Off is the kill switch: no new game can start in the mode, testers included, and games already started play on. A rollback to a release older than the Modes tab ignores the console's choices and closes every mode but Classic until the newer release returns.
