...
- **Expensive reads run off the game loop.** Statistics and the retention report run in a worker thread on a read-only SQLite connection and are cached (five and ten minutes). On a 106 MiB, 6,440-row journal the inline queries delayed a 5 ms timer by up to 29 ms; in the worker, by at most 1.2 ms.
