...
Profiles offer a display name and one of twelve portraits, with a consistent thin frame. Coral, cyan, lilac and gold portrait outlines match the assigned piece colors. Larger names sit above a light VP plaque in the caption, alongside card counts. Distinct road/army badges and a current-turn ribbon remain; a static red distressed overlay and Wi-Fi-off symbol identify disconnected portraits. There are no separate accent or frame choices.
...
[Board.tsx](../apps/client/src/Board.tsx) draws roads, settlements and cities in rich coral, cyan, lilac and gold, with filled roof caps, narrow dark contours and clear contact shadows. They share coordinates with rules-engine edges and vertices. Affordable legal sites show matching piece previews on hover/focus; clicking opens the separate placement confirmation.
...
Each harbor has two procedural planked bridges anchored to its eligible coastal corners and converging toward the boat. Original SVG ships have a visible hull, deck, sail and mast, with cargo icons kept on board. Specific ports show their resource and ratio; general ports use a question mark. The nine harbors retain four general and five specific trades. Procedural bridge geometry keeps the piers aligned with the actual rules-engine vertices.
