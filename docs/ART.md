...
Profiles offer a display name and one of twelve portraits, with a consistent thin frame. Coral, cyan, lilac and gold banners and portrait edges match the assigned piece colors. Large victory-point plaques and card counts, distinct road/army badges, a current-turn ribbon and a centered offline overlay carry game status. There are no separate accent or frame choices.
...
[Terrain.tsx](../apps/client/src/Terrain.tsx) draws a static, straight-down WebGL 2 terrain plane from the existing atlases. Inset hex masks feather biomes into shared sand. Shallow water, beach and foam follow the same island coast, with gentler broad curves on the continuous sea band's outer edge. Mirrored material repeats and inset atlas sampling reduce seams and neighboring-cell bleed. The SVG fallback uses the same art, mirrored repeat convention and sea-boundary constants.
...
[Board.tsx](../apps/client/src/Board.tsx) draws roads, settlements and cities in brighter coral, cyan, lilac and gold, with thinner dark edges and lighter static ground shadows. They share coordinates with rules-engine edges and vertices. Affordable legal sites show matching piece previews on hover/focus; clicking opens the separate placement confirmation.
...
Resource hand cards combine transparent art, distinct material colors and counts beneath a restrained whole-card sheen. They omit visible names/tooltips while retaining accessible names and quantities. Positive cards keep hover sound independently of reduced motion; empty cards stay still and silent. Trade sits left of Roll/End with clearer spacing between hand groups. Dice, resource trails, profile +N receipts and scene transitions are finite; reduced motion quiets spatial effects. Settings contain volume and the optional turn timer.

## Fonts, icons, and licenses

Interface icons are original SVG shapes in [GameIcons.tsx](../apps/client/src/GameIcons.tsx), with filled silhouettes, ink edges and a shared palette. Rules uses a book; connection uses Wi-Fi arcs with a slashed offline variant; fullscreen has distinct enter and exit shapes. Turn-grouped history reuses action and resource icons. The SVGs are editable repository contributions with no Lucide runtime dependency. Cinzel supplies titles and number tokens; Barlow supplies controls. Both fonts use SIL Open Font License 1.1 and are bundled locally through Fontsource.
