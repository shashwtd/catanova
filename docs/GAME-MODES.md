...
them: the Big Table turn style (`turns?: 'paired' | 'betweenTurnsBuild'`), the
Open Sea scenario (`sea?: { scenario }`) and the small variants.
...
- **Room setup** gets a "Game mode" section at the top, using the dialog's
  ...
  [Interface decisions for the build](#interface-decisions-for-the-build).
  Release A builds all of this but the turn structure, which comes with Big
  Table. The section shows only when the host may pick more than Classic or
  the room is already in another mode, so with Classic alone nothing on screen
  changes.
...
6. **Open Sea follows the same path.** Its core ships hidden, one part at a
   ...
   every tab's `rulesets`. The in-game controls for ships, gold picks and the
   robber-or-pirate choice are on the same branch, so a build of it can go to
   testers: keep `CATANOVA_MODE_TESTERS` empty until that build is deployed.
...
| Turn clock in the new phases                | Needs work        | The same `.turn-timer`, reading its deadline from whoever is owed a move. Change both its title and its visible label, which today says "Discards": "Gold picks", "Partner". Open Sea's is built: the paused clock says "Gold picks", and the picker's own 20 seconds (`turnClock.goldDeadlines`) count in the gold panel and on their card.            |
...
| Pirate                                      | Built             | `PirateShape`: a ship in the robber's colours, near-black `#172231` hull and sails, a `#c3b488` contour, one `#e7d8af` pennant stroke and the piece shadow, 1.1 times a ship, on its sea hex. No skull, no pure black and no darkened hex on water. On a harbour's sea hex it steps aside to clear the badge (`piratePlacement`).                       |
...
| Results screen                              | Needs work        | Points named by score term, built in Release A. "Longest Route" in Open Sea in the facts, the point breakdown, the standings and the medal's label, built with Open Sea's engine. A Mode fact names any mode but Classic, from the results' ruleset. Still to do: screenshot six players at 375 × 812.                                                  |
...
| Copy, history and labels                    | Needs work        | Built for Open Sea: its quick-rules section, the ship among the costs, card text ("robber or pirate", "roads or ships"), prompts, move-history names and icons, and accessible names for sea, gold, ships and the pirate. For Big Table: its guide section, a notice when it drops below five players, and the Partner's names.                         |
