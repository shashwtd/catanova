...
`'paired'` and `'betweenTurnsBuild'`, the default first), and the Open Sea
scenario (`sea?: { scenario }`) and the small variants will follow. The host's
choice is frozen into the game as `Game.turns`.
...
consulted only when the ruleset turns them on: `variants.ts` and `sea.ts` are to
be modules of their own. Big Table's turn structures changed the turn flow
...
- **Who is owed a move.** Today one player acts at a time, apart from discards
  ...
  "your move" cue read from it. Big Table added `partner` and `buildWindow`,
  and Open Sea will add `goldPick`, each with its timeout move in `timeout.ts`;
  whatever must be answered in order lists only the player whose turn it is to
  answer.
...
- **Your move.** Every "is it my move" check assumed one actor. Big Table keeps
  ...
  so the checks read them; a player owed a gold pick will need their own.
...
The restore verifier reads seats, the bank, the deck and the pieces from the
ruleset (Release A), and knows Big Table's phases, markers and windows. This is
cheap if it is done as each mode lands.
...
2. **Release B: Big Table**, testers first. The plan was two releases, the
   ...
   be a tester; the others join by code with up-to-date tabs. Test:
...
5. **Open Sea follows the same path.** Its core ships hidden, one part at a
...
| Turn clock in the new phases                | Built for Big Table | The same `.turn-timer`, reading its deadline from whoever is owed a move. Its title and visible label, which say "Discards" while discards pause the clock, say "Partner" and "Build window" in those phases. "Gold picks" comes with Open Sea.                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| Results screen                              | Needs work          | Points named by score term, built in Release A, and a Mode fact, built with Big Table. Six players already fit: checked at 375 × 812 and 1440 × 900, the list scrolls and nothing is cut off. Still to do: "Longest Route" in Open Sea in the facts, the standings and the medal's label.                                                                                                                                                                                                                                                                                                                                                                                                                  |
