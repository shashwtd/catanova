...
The #1 badge ranks the point totals in the viewer’s server-filtered snapshot. Your total already includes your Victory Point cards; opponents’ totals include public points only until the game reveals final scores. This keeps the badge consistent with the visible score without counting a card twice or revealing private cards through another player’s badge. Ties share #1, resigned players cannot lead, and profiles retain turn order.
