...
Supabase should supply the durable database and, later if useful, account authentication. Players should send game commands to the game server. Do not make browser clients arbitrate state via database writes or use a broadcast service as the only source of truth.
...
- **A domain:** optional. Use the assigned hosting hostname initially. Registrar and renewal prices vary; check the exact name before purchase.
...
- **Email/authentication:** production email delivery and abuse controls may cost extra if account login is added.
