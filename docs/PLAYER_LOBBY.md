...
Profiles open on the player's record, with editing as a separate action. Game history lists outcomes, turns, dates and expandable player scores. Only an unfinished, non-resigned seat that was not permanently left offers Return to game. You cannot use it to silently abandon another room.
...
- Games played and wins count matches that finished with a winner. Resigned players receive a completed loss when that match finishes. Abandoned matches are visibly closed with no winner and do not increase either total; unfinished matches also do not increase totals.
...
Every signed-in tab keeps a small presence socket open, and a seat at a table counts too, so an account is online while Catanova is open in any tab, in front or not. When its last tab closes it stays online for ten seconds, long enough for a reload, then goes offline and its last-seen time is recorded at that moment. Each change is pushed to confirmed friends as it happens, including a friend starting or finishing a game they can watch; the friends list still refreshes every minute to catch anything missed. Presence is shown only to confirmed friends, and last seen only to those the player allows. While a tab's socket is down, and for older clients, an HTTP check-in every 25 seconds stands in, expiring after 75 seconds. Failed refreshes clear cached presence. Presence resets after a server restart, and tabs reconnect within seconds; only last-seen times are stored.
...
Invitation notices, counts and the Friends inbox exclude both the occupied room and the resolved room preview using permanent IDs, even when the address bar contains a four-character alias. This filter also applies inside a started game, where Friends intentionally has no lobby-sharing controls. Five-second polling has a separate allowance of 120 reads per account per minute, with the existing IP budget still enforced; invitation writes remain limited to 30 per account per minute.
