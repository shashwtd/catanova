...
Open `http://127.0.0.1:3000`. Enter a name and create a table. Copy its invite link. Each friend joins with their own name in another browser tab or on their own device. The host can start with three or four seats; start order is randomized. Additional players cannot join a started match.
...
The Cards panel buys and plays development cards. Cards bought this turn wait until a later turn, except Victory Point cards, which count automatically. Longest Road, Largest Army, and own-turn victory are computed by the server. A finished table remains saved; create a new room for another game.
...
`npm run dev` builds the client and watches server sources. For client hot reload, keep that process running and run `npm run dev:client` separately, then open `http://127.0.0.1:5173`. Its proxy forwards WebSockets to the game server. Use `.env.example`'s origins if a reverse proxy changes the browser origin.
