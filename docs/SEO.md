...
7. Once the wiki is hosted, link it prominently from the guide and in-game help, add its sitemap to Search Console and keep article titles/links stable. Do not publish the same full encyclopedia on both Fandom and our domain as an SEO experiment.
...
## Fandom versus our own wiki

My recommendation is **the branded wiki**. Fandom's existing network can be a discovery surface, but there is no evidence that an empty new Fandom community would rank better than an equally helpful Catanova wiki. Google describes page-level ranking systems alongside site-wide signals; a host's reputation does not guarantee a new page a position. Fandom itself recommends useful articles and normal SEO work. [Google ranking systems](https://developers.google.com/search/docs/appearance/ranking-systems-guide), [Fandom SEO guidance](https://community.fandom.com/wiki/Help:Search_Engine_Optimization).

For this project, keeping a stable official address and control over the reading experience is the better long-term tradeoff. That is a recommendation, not a measured ranking result. The prepared [MediaWiki content pack](wiki/README.md) is portable, so selecting the host does not require rewriting the articles.
