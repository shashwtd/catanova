 ...
 * Big Table ('partner'), a between-turns build window ('buildWindow') and an Open Sea gold pick ('goldPick').
 * A kind is added together with its timeout move (timeout.ts), because the turn clock and the absence rule
 * act on whatever this list says, and a kind they cannot finish would halt automatic play in that room.
 */
export type OwedKind =
  'setupSettlement' | 'setupRoad' | 'roll' | 'actions' | 'robber' | 'freeRoads' | 'discard';
