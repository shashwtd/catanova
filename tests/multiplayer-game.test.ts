...
async function until(check: () => boolean) {
  ...
  while (!check()) {
    if (Date.now() > end) throw new Error('Timed out waiting for multiplayer state');
    await pause(10);
  }
}
test('four real clients finish setup, keep hands private, and recover an accepted dice roll after restart and page reload', async (t) => {
  const directory = await mkdtemp(join(tmpdir(), 'catanova-game-'));
  ...
  const a = new Connection(server.url, newSession('First'), { minRetryMs: 30, maxRetryMs: 100 });
  clients.push(a);
  a.start();
  await until(() => a.status === 'connected');
  for (let i = 1; i < 4; i++) {
    const c = new Connection(server.url, newSession(`Player ${i + 1}`, a.session.roomId), {
      minRetryMs: 30,
      maxRetryMs: 100,
    });
    clients.push(c);
  ...
  for (const c of clients) await c.lobby(true);
  await until(() => clients.every((c) => c.state!.players.every((p) => p.ready)));
  await a.action({ kind: 'start' });
  await until(() => clients.every((c) => !!c.state?.game));
  for (let i = 0; i < 16; i++) {
    const g = a.state!.game!;
    const actor = clients.find((c) => c.playerId === g.players[g.active]!.id)!;
    ...
    const ack = await actor.action(action);
    await until(() => clients.every((c) => c.state!.revision === ack.revision));
  }
  for (const c of clients) {
    ...
    for (const p of c.state!.game!.players) {
      assert.equal(!!p.hand, p.id === c.playerId);
      assert.equal(!!p.cards, p.id === c.playerId);
    }
    assert.ok(!JSON.stringify(c.state).includes('"deck":'));
  }
  const g = a.state!.game!,
    actor = clients.find((c) => c.playerId === g.players[g.active]!.id)!;
  const command: PendingCommand = {
    type: 'action',
    action: { kind: 'roll' },
    expectedRevision: actor.state!.revision,
    commandId: 'persisted-dice-command',
  };
  ...
  assert.equal(reloaded.playerId, actor.playerId);
  assert.deepEqual(reloaded.state!.game!.dice, saved.dice);
...
test('game writes roll back on storage failure and started three-player rooms refuse new seats', async (t) => {
  ...
  const hostSession = newSession('Host');
  const host = server.store.enter('create', hostSession.token, hostSession.name);
  for (const name of ['Second', 'Third']) {
    const s = newSession(name);
    server.store.enter('join', s.token, name, host.room_id);
  }
  const revision = readyLobby(server.store, host.room_id);
  server.store.db.exec('PRAGMA query_only = ON');
  assert.throws(() => server.store.action(host, 'failed-start-command', revision, { kind: 'start' }));
  assert.equal(server.store.snapshot(host.room_id).revision, revision);
  assert.equal(server.store.loadGame(host.room_id), undefined);
  server.store.db.exec('PRAGMA query_only = OFF');
  server.store.action(host, 'failed-start-command', revision, { kind: 'start' });
  const fourth = newSession('Fourth');
  assert.throws(() => server.store.enter('join', fourth.token, fourth.name, host.room_id), /already started/);
