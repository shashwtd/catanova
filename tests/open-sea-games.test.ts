...
function playGame(players: number, seed: number) {
  ...
  // Each move, and for a gold pick whether it was the one the clock would have made (section 9.5).
...
const lines = (g: Game, pattern: RegExp) => g.log.filter((line) => pattern.test(line.text)).length;
...
test('three and four players play Open Sea to the end across seeds, every card and piece accounted for', () => {
  ...
  const logs = games.map(({ g }) => g);
  assert.ok(
    logs.some((g) => lines(g, /moved the pirate and stole a card from/) > 0),
    'the pirate robs',
  );
  assert.ok(
    logs.some((g) => lines(g, /settled a new island/) > 0),
    'islands are settled',
  );
  assert.ok(
    logs.some((g) => lines(g, /claimed Longest Route/) > 0),
    'Longest Route changes hands',
  );
  assert.ok(
    logs.some((g) => lines(g, /took .* from the bank for gold/) > 0),
    'gold picks are logged',
  );
