...
test('the six-seat stylesheets load after the layered ones', () => {
  ...
  // After table-light.css, the last of the layered sheets, with the other components' own sheets.
  const after = sheets.slice(sheets.indexOf('table-light.css') + 1);
  assert.deepEqual(
    after.filter((sheet) => sheet!.startsWith('six-seat-')),
    ['six-seat-rail.css', 'six-seat-trade.css', 'six-seat-robber.css'],
  );
  assert.equal(sheets.filter((sheet) => sheet!.startsWith('six-seat-')).length, 3);
});
